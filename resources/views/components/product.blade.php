<article class="bg-white border-2 relative p-4 rounded-lg border-slate-100 cursor-pointer hover:border-[#4CAE87]" :key="index">
    <div class="w-full mb-4">
        ...
        @endphp
        @if($firstImage)
            <img class="h-[180px] w-full object-contain" src="{{ $firstImage }}" alt="{{ $firstImage }}"> <!-- Display the first image -->
        @endif
    </div>
    ...
    <div class="flex items-center justify-between">
        <p class="text-xl font-bold text-[#4CAE87]">${{ $product['pricing'] }} <span class="line-through text-xs text-slate-500 font-normal">${{ $product['pricing'] }}</span></p>
        <!-- <Button /> -->
